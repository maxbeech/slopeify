...
export default function WallDiagram({ r }: { r: DesignResult }) {
  ...
  return (
    <svg viewBox={`0 0 ${VW} ${VH}`} className="h-auto w-full" role="img"
      ...
      <rect x={wallX} y={Y(H)} width={Math.max(5, 0.8 * s)} height={Y(0) - Y(H)} fill="#94a3b8" opacity="0.7" />
      {/* Leveling pad */}
      <rect x={X(1)} y={baseY} width={(B + 0.5) * s} height={Math.max(4, 0.4 * s)} fill="#64748b" />
      ...
      {/* Height dimension */}
      <line x1={X(0.6)} y1={Y(0)} x2={X(0.6)} y2={Y(H)} stroke="#0f766e" strokeWidth="1" />
      <text x={X(0.6) - 4} y={Y(H / 2)} fill="#0f766e" fontSize="11" textAnchor="end">{H}′ H</text>
      {/* Base dimension */}
      <line x1={wallX} y1={Y(-0.2) + 14} x2={wallX + B * s} y2={Y(-0.2) + 14} stroke="#0f766e" strokeWidth="1" />
      <text x={wallX + (B * s) / 2} y={Y(-0.2) + 26} fill="#0f766e" fontSize="11" textAnchor="middle">{B}′ base</text>
    </svg>
  );
}
